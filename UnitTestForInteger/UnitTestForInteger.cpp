...
#define TEST_INTEGER_COM_BASE(num1, num2, op, res, linfo) do{ \
	if(res) { \
		...
		template <typename T>
		void TestIntegerParseBase(T num, const std::string &str, NumberIntegerParseResult res, __LineInfo* pLineInfo) {
			Integer a(num), b;
			NumberIntegerParseResult res2 = b.Parse(str);
		...
		void TestIntegerParseRight(T num, const std::string &str, __LineInfo* pLineInfo) {
			TestIntegerParseBase(num, str, Number_Integer_Parse_OK, pLineInfo);
		}

		void TestIntegerParseWrong(const std::string &str, __LineInfo* pLineInfo) {
			TestIntegerParseBase(0, str, Number_Integer_Parse_Failed, pLineInfo);
		}
